...
class LocationDetailViewController: UIViewController {
    ...
    func loadLocations() {
        guard let locationsEncoded = UserDefaults.standard.value(forKey: "weatherLocations") as? Data else {
            print("ERROR: Could not load weatherLocations data from UserDefaults. This woild always be the case the first time an app is installed, so if's that the case, ignore this error")
            return
        }
